...
class picketMap():
    ...
    def generate_plot(self,active_pickets=None):
        fig = plt.figure(figsize=(8, 8)) #TODO: Hardcoded figure 2, how do we know how many figures are on display? can we not hardcode this somehow?
        ...
        for lon, lat, dist in self.circle_text_info:
            ...
            if self.south<lat<self.north:
                ax.text(lon,lat,str(int(dist))+' km',fontsize=8,transform=ccrs.Geodetic())
